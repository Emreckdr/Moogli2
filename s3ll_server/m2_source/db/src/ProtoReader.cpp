...
int get_Item_SubType_Value(unsigned int type_value, string inputString)
{
...
#ifdef AZURA_PROTO_UPDATE
				,"WEAPON_CLAW", "WEAPON_QUIVER", "WEAPON_BOUQUET", "WEAPON_UNKNOW"
#endif	
				};
...
#ifdef AZURA_PROTO_UPDATE
				,"USE_CHANGE_COSTUME_ATTR", "USE_COSTUME_ENCHANT", "USE_COSTUME_TRANSFORM", "USE_RESET_COSTUME_ATTR", "USE_SELECT_ATTRIBUTE", "USE_FLOWER", "USE_UNKNOWN_TYPE"
#endif
				};
	...
	static int arNumberOfSubtype[_countof(arSubType)] = {
		...
		0,
#ifdef AZURA_PROTO_UPDATE
		sizeof(arSub35)/sizeof(arSub35[0]),		// 35 GrowthPet
...
{
	string arType[] = { "MONSTER", "NPC", "STONE", "WARP", "DOOR", "BUILDING", "PC", "POLYMORPH_PC", "HORSE",
#ifdef AZURA_PROTO_UPDATE
						"PET",
#endif
						"GOTO" };
...
int get_Mob_RaceFlag_Value(string inputString)
{
	string arRaceFlag[] = {"ANIMAL","UNDEAD","DEVIL","HUMAN","ORC","MILGYO","INSECT","FIRE","ICE","DESERT","TREE",
		"ATT_ELEC","ATT_FIRE","ATT_ICE","ATT_WIND","ATT_EARTH","ATT_DARK","ATT_HIDRA","METIN"};
...
bool Set_Proto_Item_Table(TItemTable *itemTable, cCsvTable &csvTable,std::map<int,const char*> &nameMap)
{
	...
	{
		itemTable->aApplies[i].bType = dataArray[19+i*2];
		itemTable->aApplies[i].lValue = dataArray[20+i*2];
	}
