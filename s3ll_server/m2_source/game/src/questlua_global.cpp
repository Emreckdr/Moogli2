...
#define NAN (INFINITY-INFINITY)
	...
	ALUA(_get_item_attr_norm_table)
	{
		...
		lua_newtable(L);
		for (TItemAttrMap::const_iterator it1=pIAM.begin(), ti1=pIAM.end(); it1!=ti1; it1++)
		{
			const DWORD dwApplyIndex = it1->first;
			const TItemAttrTable & pIAT = it1->second;
			lua_pushnumber(L, dwApplyIndex);
			lua_newtable(L);
			DWORD nIdx = 1;
			for (DWORD dwIdx = 0; dwIdx < ITEM_ATTRIBUTE_MAX_LEVEL; dwIdx++)
			{
				lua_pushnumber(L, pIAT.lValues[dwIdx]);
				lua_rawseti(L, -2, nIdx++);
			}
			for (DWORD dwIdx = 0; dwIdx < ATTRIBUTE_SET_MAX_NUM; dwIdx++)
			{
