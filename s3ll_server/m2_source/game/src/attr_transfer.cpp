...
bool AttrTransfer_make(LPCHARACTER ch)
{
	...
	if (items[1]->GetVnum() == 41324 || items[2]->GetVnum() == 41325){
		ch->ChatPacket(CHAT_TYPE_INFO, "Bu kostume bonus aktarimi yapamam");
		return false;
	}
...
void AttrTransfer_add_item(LPCHARACTER ch, int w_index, int i_index)
{
	...
	if (w_index == 2)
	{
		firstItem = ch->GetAttrTransferItem();
		if (firstItem[1] != NULL)
		{
			if (item->GetID() == firstItem[1]->GetID())
				return;
		}
		else
			return;
	...
	if (w_index > 0 && !has_attr)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("This costume doesn't have any bonus!"));
		return;
	}
	...
	/*if (w_index == 2 && attr_transfer_item[1] == NULL)
		return;*/
