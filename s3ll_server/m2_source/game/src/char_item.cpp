...
enum enum_RefineScrolls
{
	...
	SOUL_SCROLL = 7,//Buradaki 7'nin sıraya gore de?i?mesi gerekebilir.
#endif
};
...
#ifdef ENABLE_ITEM_SOUL_SYSTEM
		|| pkItemScroll->GetValue(0) == SOUL_SCROLL
#endif
		)
	{
...
void CHARACTER::__OpenPrivateShop()
{
	...
	switch (bodyPart)
	{
		...
		case 2:
			...
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("GameMaster pazar kuramaz."));
			}
			else
			{
...
void CHARACTER::__OpenPrivateShopKasmir()
{
...
void CHARACTER::UseSilkBotaryReal(const TPacketMyshopPricelistHeader* p)
{
	...
	#ifdef __ENABLE_SHOP_DECORATION_SYSTEM__
	if(m_bKasmirPaketDurum)
		__OpenPrivateShopKasmir();
	else
		__OpenPrivateShop();
	#else
	__OpenPrivateShop();
...
#ifdef __ENABLE_SHOP_DECORATION_SYSTEM__
void CHARACTER::UseSilkBotaryKasmir(void)
{
	if (m_bNoOpenedShop)
	{
		DWORD dwPlayerID = GetPlayerID();
		...
		case ITEM_CAMPFIRE:
			{
				...
				{
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("?아이템 입니다."));
					return false;
				}
				...
				{
					ChatPacket(CHAT_TYPE_INFO, "<Sistem> Bunu yapabilmek i?n 60 saniye beklemelisin!");
					return false;
				}
							...
							if (item->GetSubType() == USE_SPECIAL)
							{
								...
								switch (item->GetVnum())
								{
									...
									case 71260:
										if (IS_BOTARYABLE_ZONE(GetMapIndex()) == true)
										{
											m_bKasmirPaketDurum = true;
											UseSilkBotaryKasmir();
										}
										else
										{
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("°³AI ≫oA¡A≫ ¿­ ¼o ¾ø´A Ao¿ªAO´I´U"));
										}
										break;
#endif
								}
...
/*****************************************/
							...
							case FUCKING_BRAZIL_ITEM_AUTO_HP_RECOVERY_S:
								{
									...
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Sistem> Bunu yapabilmek icin 5 saniye beklemelisin!"));
										return false;
									}
					...
					case USE_ABILITY_UP:
						{
							...
							{
								case APPLY_MOV_SPEED:
									AddAffect(AFFECT_MOV_SPEED, POINT_MOV_SPEED, item->GetValue(2), AFF_MOV_SPEED_POTION, INFINITE_AFFECT_DURATION, 0, true);
								...
								case APPLY_ATT_SPEED:
									AddAffect(AFFECT_ATT_SPEED, POINT_ATT_SPEED, item->GetValue(2), AFF_ATT_SPEED_POTION, INFINITE_AFFECT_DURATION, 0, true);
#ifdef ENABLE_EFFECT_EXTRAPOT
            ...
            if (item2->GetType() == ITEM_SOUL)
            {
				...
				{
					if (item2->GetType() == ITEM_PENDANT && item2->GetSubType() == ARMOR_PENDANT)
					{
				...
				else if (item2->GetType() == ITEM_PENDANT && item2->GetSubType() == ARMOR_PENDANT)
				{
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("SADECE_TILSIMLARDA_KULLANILIR"));
					return false;
				}
									...
									else
									{
										...
										if (item->GetVnum() == 71151 || item->GetVnum() == 76023)
										{
											if ((item2->GetType() == ITEM_WEAPON)
												|| (item2->GetType() == ITEM_ARMOR && item2->GetSubType() == ARMOR_BODY && item2->GetSubType() == ARMOR_PENDANT))
											{
												bool bCanUse = true;
												for (int i = 0; i < ITEM_LIMIT_MAX_NUM; ++i)
												{
