...
bool CShopManager::CreateOfflineShop(LPCHARACTER owner, const char *szSign, const std::vector<TShopItemTable *> pTable, DWORD id, DWORD kasmirBaslik, DWORD kasmirNPC)
{
	...
	sprintf(szQuery, "insert into player_shop set item_count=%d,player_id=%d,name=\"%s\",map_index=%d,x=%ld,y=%ld,z=%ld,ip='%s',date=NOW(),date_close=FROM_UNIXTIME(%d),channel=%d,kasmirbaslik=%d,kasmirnpc=%d",
		pTable.size(), owner->GetPlayerID(), szSign, map_index, x, y, z, inet_ntoa(owner->GetDesc()->GetAddr().sin_addr), date_close, g_bChannel, kasmirBaslik, kasmirNPC);
...
bool CShopManager::StartOfflineShop(DWORD id, bool onboot)
{
	...
	DWORD kasmirbaslik, kasmirnpc;
#endif
	SQLMsg * pkMsg(DBManager::instance().DirectQuery("SELECT player_shop.name,player_shop.player_id,player.name as player_name,player_shop.map_index,player_shop.x,player_shop.y,player_shop.z,UNIX_TIMESTAMP(player_shop.date_close),player_shop.kasmirbaslik,player_shop.kasmirnpc,player_shop.id from player_shop left join player on player.id=player_shop.player_id where player_shop.id='%d'", id));
	SQLResult * pRes = pkMsg->Get();
	if (pRes->uiNumRows>0)
	{
		MYSQL_ROW row;
...
#ifdef __ENABLE_SHOP_DECORATION_SYSTEM__
			str_to_number(kasmirbaslik, row[8]);
			str_to_number(kasmirnpc, row[9]);
#endif
		}
	}
	...
	if (ch)
	{
		...
		ch->OpenShop(id, name.c_str(), kasmirbaslik, kasmirnpc, onboot);
...
{
#ifdef __ENABLE_SHOP_DECORATION_SYSTEM__
	LPSHOP pkShop = FindPCShop(ch->IsPrivShop() && (ch->GetRaceNum() >= 30000 && ch->GetRaceNum() <= 30008) ? ch->GetPrivShop() : ch->GetVID());
	...
	m_map_pkShopByPC.erase(ch->IsPrivShop() && (ch->GetRaceNum() >= 30000 && ch->GetRaceNum() <= 30008) ? ch->GetPrivShop() : ch->GetVID());
#endif
	M2_DELETE(pkShop);
}
#else
