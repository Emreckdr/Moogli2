...
void CHARACTER::EncodeInsertPacket(LPENTITY entity)
{
	...
	if (IsPC() == true || m_bCharType == CHAR_TYPE_NPC)
	{
		...
		{
		show_all_info:
	#if defined(SHOP_HIDE_NAME) && defined(__ENABLE_SHOP_DECORATION_SYSTEM__)
		if (IsPC() || (GetRaceNum() >= 30000 && GetRaceNum() <= 30008) && !IsPrivShop())
	#endif
...
void CHARACTER::OpenMyShop(const char * c_pszSign, TShopItemTable * pTable, BYTE bItemCount, DWORD id, DWORD KasmirNpc, DWORD KasmirBaslik)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("°©zER?atl???R?»ó?R?z­ L?RÖ¨R??."));
		return;
	}
	...
	{
		//sys_err("[OVERFLOW_GOLD] Overflow (GOLD_MAX) id %u name %s", GetPlayerID(), GetName());
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("20ld l R?AE°ú?z?»ó?R?z­Lö°^ lr???"));
		return;
	}
#ifdef ENABLE_OFFLINE_SHOP_USE_CHEQUE
	if (CHEQUE_MAX <= nTotalCheque)
	...
	for (BYTE i = 0; i < bItemCount; ++i)
	{
		...
		if (pkItem)
		{
			...
			if (item_table && (IS_SET(item_table->dwAntiFlags, ITEM_ANTIFLAG_GIVE | ITEM_ANTIFLAG_MYSHOP)))
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT( "RZ·áC­ lCRELURþ ?RÎ»óÁ^z^L­ CÇ¸L? L?lr???."));
				return;
			}

			if (pkItem->IsEquipped())
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT( "RlþnÁßR?lCRELURþ ?RÎ»óÁ^z^L­ CÇ¸L? L?lr???."));
				return;
			}

			if (pkItem->isLocked())
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT( "»çzëÁ??lCRELURþ ?RÎ»óÁ^z^L­ CÇ¸L? L?lr???."));
				return;
			}
	...
	if (KasmirNpc < 30000 && KasmirNpc > 30008) 
		KasmirNpc = 30000;
	...
	if (m_bKasmirPaketDurum == true) 
	{
		...
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Marketinde baslik yok!"));
			return;
		}
	}
#endif
	...
	if (CBanwordManager::instance().CheckString(m_stShopSign.c_str(), m_stShopSign.length()) != "")
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT( "þnL???Rþlî°^ C÷ÇÔµC »ó? RE¸§R¸·?»ó?R?z­ L?lr???."));
		return;
	}
#else
	if (CBanwordManager::instance().CheckString(m_stShopSign.c_str(), m_stShopSign.length()))
	...
	{
#ifdef __ENABLE_SHOP_DECORATION_SYSTEM__
		CShopManager::instance().CreateOfflineShop(this, m_stShopSign.c_str(), map_shop, id, KasmirBaslik, KasmirNpc);
#endif
		m_stShopSign.clear();
		return;
	}
...
#ifdef __ENABLE_SHOP_DECORATION_SYSTEM__
void CHARACTER::OpenShop(DWORD id, const char *name, DWORD kasmirbaslik, DWORD kasmirnpc, bool onboot)
#endif
{
	if (GetMyShop())
	{
...
#ifdef __DEFENSE_WAVE__
	if (IsDefanceWaweMastAttackMob(GetRaceNum()))
		return m_pkMobData->m_table.dwDamageRange[1] + 1000;
	else
...
bool CHARACTER::RequestToParty(LPCHARACTER leader)
{
	...
	switch (errcode)
	{
		...
		case PERR_LVBOUNDARY:
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<ÆÄÆ¼> -30 ~ +30 ·¹º§ ÀÌ³»ÀÇ »ó´ë¹æ¸¸ ÃÊ?ëÇ?¼ö ÀÖ½À´Ï´Ù."));
			return false;
...
#ifdef OFFLINE_SHOP
	if (IsPC() || IsNPC() && (GetRaceNum() >= 30000 && GetRaceNum() <= 30008))
	{		
		if (!CTargetManager::instance().GetTargetInfo(pkChrCauser->GetPlayerID(), TARGET_TYPE_VID, GetVID()) || GetRaceNum() >= 30000 && GetRaceNum() <= 30008)
#else
...
#endif
		{
			if (GetMyShop())
			{
				...
				if (pkChrCauser == this)
				{
					...
					{
						pkChrCauser->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("?? ??Áß(A^°í,?CZ,»ó?)z^´Â ?RÎ»óÁ^R?»çzëÇN L?lr???."));
						return;
					}
