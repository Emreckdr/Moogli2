...
class ItemToolTip(ToolTip):
	...
	AFFECT_DICT = {
		...
		item.APPLY_ATTBONUS_HIDRA : localeInfo.TOOLTIP_APPLY_ATTBONUS_HIDRA,
	...
	def SetInventoryItem(self, slotIndex, window_type = player.INVENTORY):
		...
		if player.GetItemCount(slotIndex) > 1:
			self.AppendSpace(5)
			self.AppendTextLine("|Eemoji/key_shift|e + |Eemoji/key_lclick|e - item Bolme")
	...
	if app.ENABLE_BUY_WITH_ITEM:
		def AppendPrice_WithItem(self, price, itemVnum, itemName):
			self.AppendSpace(5)
			self.AppendTextLine("[Nesne ile satýn alýnabilir]" , self.SHOP_ITEM_COLOR)
			item.SelectItem(itemVnum)
