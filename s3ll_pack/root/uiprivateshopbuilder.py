	...
	class PrivateShopAdvertisementBoardNormal(ui.ThinBoard_Darkmmo_Normal):
		...
		def OnMouseLeftButtonUp(self):
			if not self.vid:
				return
			if self.vid != player.GetMainCharacterIndex():
				self.textLine.SetFontColor(300, 300, 0)
...
class PrivateShopBuilder(ui.ScriptWindow):
	...
	def Open(self, title,days, npc_no, baslik_no):
	...
	def AcceptInputPrice(self):
		...
		if text and int(text) >= 2000000001:
			chat.AppendChat(1, "Maksimum kullanabileceiniz Yang miktar˝ 2.000.000.000'dir.")
			return TRUE
	...
	def OnOk(self):
		...
		shop.BuildPrivateShop(self.title,self.days, self.KasmirNpc, self.KasmirBaslik)
		self.Close()
