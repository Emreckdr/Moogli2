...
class Interface(object):
	...
	def __MakeExpandMoney(self):
		self.wndExpandedMoney = uiTaskBar.ExpandedMoneyTaskBar()
...
################## END GAYA SYSTEM #################
	...
	def Close(self):
		...
		if self.wndExpandedMoney:
			self.wndExpandedMoney.Destroy()
...
##################### END GAYA SYSTEM #####################
	...
	def ToggleExpandedMoneyButton(self):
		if self.wndExpandedMoney.IsShow():
			self.wndExpandedMoney.Close()
		else:
			self.wndExpandedMoney.Open()
...
################# GAYA SYSTEM ############################
	...
	def AppearPrivateShop(self, vid, baslik, text):
		if baslik == 1:
		...
		if constInfo.ENABLE_COLOR_SCROLL and ('#' in name):
			for s_color in ("green","blue","purple"):
				if name.endswith(s_color):
					btn.SetUpVisual("locale/tr/icon/scroll_close_%s.tga"%s_color)
					btn.SetOverVisual("locale/tr/icon/scroll_open_%s.tga"%s_color)
					btn.SetDownVisual("locale/tr/icon/scroll_open_%s.tga"%s_color)
