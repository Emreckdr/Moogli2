...
class InventoryWindow(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		try:
			...
			self.StloverButton = self.GetChild2("StloverButton")
			...
			self.equipmentTab.append(self.GetChild("Inventory_Tab_03"))
			self.equipmentTab.append(self.GetChild("Inventory_Tab_04"))
		...
		# STLOVER - Pazar System
		if self.StloverButton:
			self.StloverButton.SetEvent(ui.__mem_func__(self.ClickStloverButton))
	...
	def RefreshBagSlotWindow(self):
		...
		for i in xrange(player.INVENTORY_PAGE_SIZE):
			...
			if app.ENABLE_ITEM_SOUL_SYSTEM:
				if player.GetItemTypeBySlot(slotNumber) == item.ITEM_TYPE_SOUL:
					metinSocket = [player.GetItemMetinSocket(slotNumber, j) for j in xrange(player.METIN_SOCKET_MAX_NUM)]
	...
	def UseItemSlot(self, slotIndex):
		...
		if app.IsPressed(app.DIK_LCONTROL) and app.IsPressed(app.DIK_X):
			...
			net.SendItemSellPacket(slotIndex)
			snd.PlaySound("sound/ui/money.wav")
			return
