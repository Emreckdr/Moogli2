...
class SelectCharacterWindow(ui.Window):
	...
	RACE_NAME = {
					...
					playerSettingModule.RACE_WOLFMAN_M : localeInfo.JOB_WOLFMAN,
	}
	...
	def __LoadBoardDialog(self, fileName):
		...
		GetObject("CharacterFace_0").OnMouseOverIn = lambda arg=5: self.OverInButton(arg)
		GetObject("CharacterFace_0").OnMouseOverOut = lambda arg=5: self.OverOutButton()
		...
		GetObject("CharacterFace_1").OnMouseOverIn = lambda arg=6: self.OverInButton(arg)
		GetObject("CharacterFace_1").OnMouseOverOut = lambda arg=6: self.OverOutButton()
		...
		GetObject("CharacterFace_2").OnMouseOverIn = lambda arg=7: self.OverInButton(arg)
		GetObject("CharacterFace_2").OnMouseOverOut = lambda arg=7: self.OverOutButton()
		...
		GetObject("CharacterFace_3").OnMouseOverIn = lambda arg=8: self.OverInButton(arg)
		GetObject("CharacterFace_3").OnMouseOverOut = lambda arg=8: self.OverOutButton()
		...
		GetObject("CharacterFace_4").OnMouseOverIn = lambda arg=9: self.OverInButton(arg)
		GetObject("CharacterFace_4").OnMouseOverOut = lambda arg=9: self.OverOutButton()
	...
	def MakeCharacter(self, index, id, name, race, form, hair, sash):
		...
		chr.SetHair(hair)
		import dbg
		#dbg.TraceError("Set sash %d"%int(sash))
		chr.SetSash(sash)
		chr.Refresh()
	...
	def ChangeSlot(self, action = "NONE"):
		if action == "UP":
			if self.slot == 0:
				self.slot = 4
			else:
	...
	def OnUpdate(self):
		...
		for i in xrange(self.SLOT_COUNT):
			...
			if id != 0:
				count += 1
		if count == 0 and constInfo.FIRST_CREATE_CHARACTER == 0:
			constInfo.FIRST_CREATE_CHARACTER = 1
			self.CreateCharacter()
