...
class CPythonNetworkStream : public CNetworkStream, public CSingleton<CPythonNetworkStream>
{
	...
	public:
		...
		bool SendBuildPrivateShopPacket(const char * c_szName, const std::vector<TShopItemTable> & c_rSellingItemStock,DWORD days, DWORD dwKasmirNpc, BYTE bKasmirBaslik);
#else
		...
		void AppearShopSign(DWORD dwVID, std::string stSign = "", DWORD bShopKasmirTitle = 0);
#endif
