...
void CPythonShop::BuildPrivateShop(const char * c_szName,DWORD days, DWORD dwKasmirNpc, BYTE bKasmirBaslik)
#else
...
#endif
{
	...
	#ifdef ENABLE_SHOP_DECORATION_SYSTEM
	CPythonNetworkStream::Instance().SendBuildPrivateShopPacket(c_szName, ItemStock,days, dwKasmirNpc, bKasmirBaslik);
	#else
...
PyObject * shopBuildPrivateShop(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (!PyTuple_GetInteger(poArgs, 1, &days))
		return Py_BuildException();
	int iKasmirNpc;
	if (!PyTuple_GetInteger(poArgs, 2, &iKasmirNpc))
		return Py_BuildException();
	BYTE bKasmirBaslik;
	if (!PyTuple_GetInteger(poArgs, 3, &bKasmirBaslik))
		return Py_BuildException();
	CPythonShop::Instance().BuildPrivateShop(szName,days,iKasmirNpc,bKasmirBaslik);
#else
