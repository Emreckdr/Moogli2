...
void CPythonNetworkStream::AppearShopSign(DWORD dwVID, std::string stSign, DWORD bShopKasmirTitle)
{
	if (stSign.empty())
		for (auto it = m_mapShopSign.begin(); it != m_mapShopSign.end(); ++it)
			if (dwVID == it->first) {
				stSign = it->second.sign;
				bShopKasmirTitle = it->second.baslik;
			}
	// LogBoxf("AppearShopSign: %u-%s", dwVID, stSign.c_str());
	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_PrivateShop_Appear", Py_BuildValue("(isi)", dwVID, stSign.c_str(), bShopKasmirTitle));
}
#endif

// MyShop
#ifdef ENABLE_SHOP_DECORATION_SYSTEM
bool CPythonNetworkStream::SendBuildPrivateShopPacket(const char * c_szName, const std::vector<TShopItemTable> & c_rSellingItemStock,DWORD days, DWORD dwKasmirNpc, BYTE bKasmirBaslik)
#else
...
bool CPythonNetworkStream::RecvShopSignPacket()
{
	...
	else
	{
		...
		if (pInstance && (pInstance->IsPC() || (pInstance->GetRace() >= 30000 && pInstance->GetRace() <= 30008)))
		{
			...
			m_mapShopSign[p.dwVID].baslik = p.bShopKasmirTitle;
			AppearShopSign(p.dwVID, p.szSign, p.bShopKasmirTitle);
