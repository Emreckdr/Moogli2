...
UINT CInstanceBase::GetNameColorIndex()
{
		...
		if (GetRace() >= 30000 && GetRace() <= 30008)
...
DWORD CInstanceBase::__AttachEffect(UINT eEftType, float)
{
	// 2004.07.17.levites.isShow를 ViewFrustumCheck로 변경
#ifdef ENABLE_CANSEEHIDDENTHING_FOR_GM
	if (IsAffect(AFFECT_INVISIBILITY) && !__MainCanSeeHiddenThing())
	...
	{
		return m_GraphicThingInstance.AttachEffectByID(0, NULL, ms_adwCRCAffectEffect[eEftType], (const D3DXVECTOR3*)NULL);
	}
	else
	{
		...
		{
			return m_GraphicThingInstance.AttachEffectByID(0, rstrBoneName.c_str(), ms_adwCRCAffectEffect[eEftType], (const D3DXVECTOR3*)NULL);
		}
	}
